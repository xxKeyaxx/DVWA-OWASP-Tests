...
def analyze_response_differences(valid_good, valid_bad, invalid_bad):
    """Analyze differences between responses to detect username policy weaknesses"""
    findings = []
    analysis_details = []
    ...
    if len(set(codes.values())) > 1:
        findings.append({
            ...
            'username': 'Multiple',
            'evidence': f"Different status codes reveal user existence: Valid+Good={codes['valid_good']}, Valid+Bad={codes['valid_bad']}, Invalid+Bad={codes['invalid_bad']}",
            'severity': 'High'
        })
    ...
    if length_diff > 50:
        findings.append({
            ...
            'username': 'Multiple',
            'evidence': f"Significant length difference reveals user existence: Valid+Bad={lengths['valid_bad']} bytes, Invalid+Bad={lengths['invalid_bad']} bytes (diff: {length_diff} bytes)",
            'severity': 'High'
        })
    ...
    if time_diff > 0.3:
        findings.append({
            ...
            'username': 'Multiple',
            'evidence': f"Timing difference reveals user existence: Valid+Bad={times['valid_bad']:.3f}s, Invalid+Bad={times['invalid_bad']:.3f}s (diff: {time_diff:.3f}s)",
            'severity': 'Medium'
        })
    ...
    if titles['valid_bad'] != titles['invalid_bad']:
        findings.append({
            ...
            'username': 'Multiple',
            'evidence': f"Different titles reveal user existence: Valid+Bad='{titles['valid_bad']}', Invalid+Bad='{titles['invalid_bad']}'",
            'severity': 'High'
        })
        ...
        if indicator in valid_bad_content:
            ...
            if indicator not in invalid_bad_content:
                findings.append({
                    ...
                    'username': 'admin',
                    'evidence': f"Response contains '{indicator}' for valid user with wrong password, but not for invalid user - reveals user existence",
                    'severity': 'High'
                })
            break
        ...
        if indicator in invalid_bad_content:
            ...
            if indicator not in valid_bad_content:
                findings.append({
                    ...
                    'evidence': f"Response contains '{indicator}' for invalid user, but not for valid user - reveals user non-existence",
                    'severity': 'High'
                })
            break
...
def test_predictable_username_patterns(session, base_url):
    """Test predictable username patterns for enumeration"""
    findings = []
    
    # Common predictable patterns
    predictable_patterns = [
        'admin', 'administrator', 'root', 'user', 'test',
        'guest', 'info', 'webmaster', 'support', 'helpdesk',
        'gordonb', 'pablo', 'smithy'
    ]
    ...
    # Test known valid DVWA users
    for username in ['admin', 'gordonb', 'pablo', 'smithy']:
        try:
            response_data, error = test_login_response(session, base_url, username, 'wrongpassword123')
            
            if not error and response_data:
                # Check if response suggests user exists
    ...
    # Test common invalid patterns
    for username in ['nonexistentuser', 'invaliduser123', 'testuser999']:
        try:
            response_data, error = test_login_response(session, base_url, username, 'wrongpassword123')
            
            if not error and response_data:
                # Check if response suggests user doesn't exist
                content = response_data['content'].lower()
                user_not_exists_indicators = [
                    'user not found', 'invalid username', 'username not found',
                    'no such user', 'unknown user'
    ...
    if valid_users:
        findings.append({
            'vulnerability': 'Predictable Username Structure',
            'username': f"Found {len(valid_users)} valid users",
            'evidence': f"Valid users with predictable names: {', '.join(valid_users[:3])}",
            'severity': 'Low'
        })
    
    return findings

def test_username_structure_analysis(session, base_url):
    """Analyze username structure for predictability"""
    findings = []
    
    # Known DVWA usernames
    known_usernames = ['admin', 'gordonb', 'pablo', 'smithy']
    
    print("[*] Analyzing username structure...")
    ...
    for username in known_usernames:
        ...
        if username.lower() in ['admin', 'administrator', 'root']:
            if 'Admin/Root Pattern' not in patterns_found:
                patterns_found.append('Admin/Root Pattern')
                
        # Check for short usernames
        if len(username) < 6:
            if 'Short Username Pattern' not in patterns_found:
                patterns_found.append('Short Username Pattern')
                
        # Check for simple alphabetical patterns
        if username.isalpha() and len(username) < 8:
            if 'Simple Alphabetic Pattern' not in patterns_found:
                patterns_found.append('Simple Alphabetic Pattern')
    
    if patterns_found:
        findings.append({
            'vulnerability': 'Weak Username Structure',
            'username': 'Multiple',
            'evidence': f"Identified weak username patterns: {', '.join(patterns_found)}",
            'severity': 'Low'
        })
    
    return findings

def test_inconsistent_error_messages(session, base_url):
    """Test if error messages are INCONSISTENT (which would be bad)"""
    findings = []
    ...
    test_cases = [
        ('', 'empty123'),  # Empty username
        ('admin', ''),     # Empty password
        ('', ''),          # Both empty
        (' ', 'test123'),  # Space username
    ...
    # Check if all responses are INCONSISTENT (this would be a vulnerability)
    if len(responses) > 1:
        lengths = [r['content_length'] for r in responses]
        titles = [r['title'] for r in responses]
        
        if len(set(lengths)) > 1 or len(set(titles)) > 1:
            findings.append({
                'vulnerability': 'Inconsistent Error Messages',
                'username': 'Multiple',
                'evidence': 'Error responses vary in content length or titles - may enable enumeration',
                'severity': 'Medium'
            })
        # Note: Consistent responses are good security practice, so we don't report them as findings
    
    return findings

def generate_html_report(findings, analysis_details, base_url):
...
<html>
<head>
    <title>Security Assessment Report - OTG-IDENT-005</title>
    <style>
        body {{
...
<body>
    <div class="header">
        ...
        <p><strong>Test ID:</strong> OTG-IDENT-005 - Weak or Unenforced Username Policy</p>
        <p><strong>Date:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
    </div>

    <div class="section">
        <h2 class="section-title">Executive Summary</h2>
        <p>This assessment evaluated the username policy enforcement of the DVWA application 
        according to OWASP Testing Guide v4 - OTG-IDENT-005. The test focused on identifying 
        weaknesses in account enumeration resistance and inconsistent error message handling 
        that could reveal information about valid usernames.</p>
    ...
    <div class="section">
        ...
        <p>The testing methodology followed OWASP Testing Guide v4 guidelines for testing weak username policies:</p>
        <ul>
            <li>Analysis of login response for valid user + valid password</li>
            <li>Analysis of login response for valid user + wrong password</li>
            <li>Analysis of login response for invalid user + invalid password</li>
            <li>Comparison of response characteristics for account enumeration</li>
            <li>Testing predictable username patterns</li>
            <li>Analysis of username structure predictability</li>
            <li>Evaluation of error message consistency</li>
        </ul>
    </div>
    ...
    html_content += f"""
            <p>Analysis of response differences helps identify potential username enumeration vulnerabilities.</p>
        </div>
    </div>
    ...
    if findings:
        ...
        html_content += """
            <tr>
                <td colspan="4" style="text-align: center;">No username policy weaknesses found - good security practice</td>
            </tr>"""
    ...
    <div class="section">
        ...
        <p>Weak or unenforced username policies can lead to account enumeration attacks where 
        attackers can identify valid usernames and subsequently perform targeted brute force 
        or password spraying attacks. Consistent error messages and response handling are 
        essential for preventing such enumeration.</p>
    </div>

    <div class="section">
        <h2 class="section-title">Recommendations</h2>
        <ol>
            <li>Ensure consistent error messages for all authentication failures</li>
            <li>Maintain uniform response times regardless of username validity</li>
            <li>Use identical response content length for all login attempts</li>
            <li>Avoid revealing whether a username exists through error messages</li>
            <li>Implement account lockout mechanisms after failed login attempts</li>
            <li>Discourage predictable username structures</li>
            <li>Monitor and log authentication attempts for suspicious activity</li>
            <li>Consider implementing CAPTCHA for login forms after several failed attempts</li>
        </ol>
    </div>
    ...
    <div class="section">
        ...
        <ul>
            <li>OWASP Testing Guide v4 - OTG-IDENT-005: Testing for Weak or Unenforced Username Policy</li>
    ...
    # Save the report
    with open("OTG-IDENT-005_Report.html", "w", encoding="utf-8") as f:
        f.write(html_content)
    
    print("HTML report generated: OTG-IDENT-005_Report.html")
...
def main():
    ...
    print("[*] Starting OTG-IDENT-005 Username Policy Test")
    ...
    # Perform the core tests as per OWASP methodology
    print("[*] Performing core OWASP username policy tests...")
    ...
    print("[*] Testing predictable username patterns...")
    predictable_findings = test_predictable_username_patterns(session, base_url)
    all_findings.extend(predictable_findings)
    
    print("[*] Analyzing username structure...")
    structure_findings = test_username_structure_analysis(session, base_url)
    all_findings.extend(structure_findings)
    
    print("[*] Testing error message consistency...")
    consistency_findings = test_inconsistent_error_messages(session, base_url)
    all_findings.extend(consistency_findings)
    
    # Generate report
    print("[*] Generating HTML report...")
    generate_html_report(all_findings, analysis_details, base_url)
    ...
    if len(all_findings) == 0:
        print("✅ NO USERNAME POLICY WEAKNESSES FOUND")
        print("   DVWA demonstrates good username policy enforcement")
    else:
    ...
    print("="*60)
    print("Report saved as: OTG-IDENT-005_Report.html")
